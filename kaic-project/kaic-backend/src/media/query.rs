...
pub fn build_messages(task_text: &str) -> Vec<Message> {
    vec![
        Message::new(
            Role::System,
            "Ты выделяешь поисковый запрос для банка фотографий. \
...
pub fn build_request(task_text: &str) -> GenerateRequest {
    GenerateRequest {
        ...
        temperature: None,
        // Инструментов у этого шага нет и быть не должно: извлечение
        // поискового термина — одна реплика модели, а не агентный цикл.
        // Пустой список гарантирует побайтово прежний запрос.
        tools: Vec::new(),
    }
}
